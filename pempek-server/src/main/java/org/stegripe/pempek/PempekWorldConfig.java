...
import org.stegripe.pempek.tool.Flattenable;
import org.stegripe.pempek.tool.Strippable;
import org.stegripe.pempek.tool.Tillable;
import org.stegripe.pempek.tool.Waxable;
import org.stegripe.pempek.tool.Weatherable;

import static org.stegripe.pempek.PempekConfig.log;

@SuppressWarnings("unused")
public class PempekWorldConfig {

    private final String legacyWorldName;
    private final String worldName;
    private final World.Environment environment;

    public PempekWorldConfig(String legacyWorldName, World.Environment environment, Key worldKey) {
    ...
    public void init() {
        if (PempekConfig.version < 48) {
            ConfigurationSection section = PempekConfig.config.getConfigurationSection("world-settings." + this.legacyWorldName);
            if (section != null) {
                PempekConfig.config.set("world-settings." + this.legacyWorldName, null);
                PempekConfig.config.set("world-settings." + this.worldName, section);
                Bukkit.getLogger().info("NOTE: Migrated Pempek world config %s -> %s".formatted(this.legacyWorldName, this.worldName));
            }
        }
        log("-------- World Settings For [" + worldName + "] --------");
        PempekConfig.readConfig(PempekWorldConfig.class, this);
    }

    private void set(String path, Object val) {
        if (PempekConfig.config.get("world-settings.default." + path) == null || val == null) {
            PempekConfig.config.addDefault("world-settings.default." + path, val);
            PempekConfig.config.set("world-settings.default." + path, val);
        }
        if (PempekConfig.config.get("world-settings." + worldName + "." + path) != null) {
            PempekConfig.config.addDefault("world-settings." + worldName + "." + path, val);
            PempekConfig.config.set("world-settings." + worldName + "." + path, val);
        }
    }

    private ConfigurationSection getConfigurationSection(String path) {
        ConfigurationSection section = PempekConfig.config.getConfigurationSection("world-settings." + worldName + "." + path);
        return section != null ? section : PempekConfig.config.getConfigurationSection("world-settings.default." + path);
    }

    private String getString(String path, String def) {
        if (PempekConfig.config.get("world-settings.default." + path) == null) {
            PempekConfig.config.addDefault("world-settings.default." + path, def);
        }
        return PempekConfig.config.getString("world-settings." + worldName + "." + path, PempekConfig.config.getString("world-settings.default." + path));
    }
    ...
    private boolean getBoolean(String path, boolean def) {
        ...
        return PempekConfig.config.getBoolean("world-settings." + worldName + "." + path, PempekConfig.config.getBoolean("world-settings.default." + path));
    }
    ...
    private double getDouble(String path, double def) {
        ...
        return PempekConfig.config.getDouble("world-settings." + worldName + "." + path, PempekConfig.config.getDouble("world-settings.default." + path));
    }
    ...
    private int getInt(String path, int def) {
        ...
        return PempekConfig.config.getInt("world-settings." + worldName + "." + path, PempekConfig.config.getInt("world-settings.default." + path));
    }
    ...
    private <T> List<?> getList(String path, T def) {
        ...
        return PempekConfig.config.getList("world-settings." + worldName + "." + path, PempekConfig.config.getList("world-settings.default." + path));
    }

    private Map<String, Object> getMap(String path, Map<String, Object> def) {
        final Map<String, Object> fallback = PempekConfig.getMap("world-settings.default." + path, def);
        final Map<String, Object> value = PempekConfig.getMap("world-settings." + worldName + "." + path, null);
    ...
    public Identifier dontRunWithScissorsItemModelReference = Identifier.parse("stegripe:scissors");
    ...
    private void itemSettings() {
        ...
        dontRunWithScissorsItemModelReference = Identifier.parse(getString("gameplay-mechanics.item.shears.damage-if-sprinting-item-model", "stegripe:scissors"));
    ...
    private void playerSettings() {
        ...
        idleTimeoutKick = System.getenv("PEMPEK_FORCE_IDLE_KICK") == null ? getBoolean("gameplay-mechanics.player.idle-timeout.kick-if-idle", idleTimeoutKick) : Boolean.parseBoolean(System.getenv("PEMPEK_FORCE_IDLE_KICK"));
    ...
    private void toolSettings() {
        ...
        if (PempekConfig.version < 18) {
            ConfigurationSection section = PempekConfig.config.getConfigurationSection("world-settings." + worldName + ".tools.hoe.tilling");
            if (section != null) {
                PempekConfig.config.set("world-settings." + worldName + ".tools.hoe.tillables", section);
                PempekConfig.config.set("world-settings." + worldName + ".tools.hoe.tilling", null);
            }
            section = PempekConfig.config.getConfigurationSection("world-settings.default.tools.hoe.tilling");
            if (section != null) {
                PempekConfig.config.set("world-settings.default.tools.hoe.tillables", section);
                PempekConfig.config.set("world-settings.default.tools.hoe.tilling", null);
            }
        }
        if (PempekConfig.version < 29) {
            PempekConfig.config.set("world-settings.default.tools.axe.strippables.minecraft:mangrove_log", Map.of("into", "minecraft:stripped_mangrove_log", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.strippables.minecraft:mangrove_wood", Map.of("into", "minecraft:stripped_mangrove_wood", "drops", new HashMap<String, Double>()));
        }
        if (PempekConfig.version < 32) {
            PempekConfig.config.set("world-settings.default.tools.axe.strippables.minecraft:cherry_log", Map.of("into", "minecraft:stripped_cherry_log", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.strippables.minecraft:cherry_wood", Map.of("into", "minecraft:stripped_cherry_wood", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.strippables.minecraft:bamboo_block", Map.of("into", "minecraft:stripped_bamboo_block", "drops", new HashMap<String, Double>()));
        }
        if (PempekConfig.version < 33) {
            getList("gameplay-mechanics.shovel-turns-block-to-grass-path", new ArrayList<String>(){{
                ...
                PempekConfig.config.set("world-settings.default.tools.shovel.flattenables." + key.toString(), Map.of("into", "minecraft:dirt_path", "drops", new HashMap<String, Double>()));
            });
            set("gameplay-mechanics.shovel-turns-block-to-grass-path", null);
        }
        if (PempekConfig.version < 34) {
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_chiseled_copper", Map.of("into", "minecraft:chiseled_copper", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_chiseled_copper", Map.of("into", "minecraft:exposed_chiseled_copper", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_chiseled_copper", Map.of("into", "minecraft:weathered_chiseled_copper", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_chiseled_copper", Map.of("into", "minecraft:oxidized_chiseled_copper", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_copper_door", Map.of("into", "minecraft:copper_door", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_copper_door", Map.of("into", "minecraft:exposed_copper_door", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_copper_door", Map.of("into", "minecraft:weathered_copper_door", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_copper_door", Map.of("into", "minecraft:oxidized_copper_door", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_copper_trapdoor", Map.of("into", "minecraft:copper_trapdoor", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_copper_trapdoor", Map.of("into", "minecraft:exposed_copper_trapdoor", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_copper_trapdoor", Map.of("into", "minecraft:weathered_copper_trapdoor", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_copper_trapdoor", Map.of("into", "minecraft:oxidized_copper_trapdoor", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_copper_grate", Map.of("into", "minecraft:copper_grate", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_copper_grate", Map.of("into", "minecraft:exposed_copper_grate", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_copper_grate", Map.of("into", "minecraft:weathered_copper_grate", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_copper_grate", Map.of("into", "minecraft:oxidized_copper_grate", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_copper_bulb", Map.of("into", "minecraft:copper_bulb", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_copper_bulb", Map.of("into", "minecraft:exposed_copper_bulb", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_copper_bulb", Map.of("into", "minecraft:weathered_copper_bulb", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_copper_bulb", Map.of("into", "minecraft:oxidized_copper_bulb", "drops", new HashMap<String, Double>()));

            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_chiseled_copper", Map.of("into", "minecraft:chiseled_copper", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_chiseled_copper", Map.of("into", "minecraft:exposed_chiseled_copper", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_chiseled_copper", Map.of("into", "minecraft:weathered_chiseled_copper", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_cut_copper_stairs", Map.of("into", "minecraft:weathered_cut_copper_stairs", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_copper_door", Map.of("into", "minecraft:copper_door", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_copper_door", Map.of("into", "minecraft:exposed_copper_door", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_copper_door", Map.of("into", "minecraft:weathered_copper_door", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_copper_trapdoor", Map.of("into", "minecraft:copper_trapdoor", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_copper_trapdoor", Map.of("into", "minecraft:exposed_copper_trapdoor", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_copper_trapdoor", Map.of("into", "minecraft:weathered_copper_trapdoor", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_copper_grate", Map.of("into", "minecraft:copper_grate", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_copper_grate", Map.of("into", "minecraft:exposed_copper_grate", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_copper_grate", Map.of("into", "minecraft:weathered_copper_grate", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_copper_bulb", Map.of("into", "minecraft:copper_bulb", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_copper_bulb", Map.of("into", "minecraft:exposed_copper_bulb", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_copper_bulb", Map.of("into", "minecraft:weathered_copper_bulb", "drops", new HashMap<String, Double>()));
        }
        if (PempekConfig.version < 39) {
            PempekConfig.config.set("world-settings.default.tools.axe.strippables.minecraft:pale_oak_wood", Map.of("into", "minecraft:stripped_pale_oak_wood", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.strippables.minecraft:pale_oak_log", Map.of("into", "minecraft:stripped_pale_oak_log", "drops", new HashMap<String, Double>()));
        }
        if (PempekConfig.version < 44) {
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_copper_chest", Map.of("into", "minecraft:copper_chest", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_copper_chest", Map.of("into", "minecraft:exposed_copper_chest", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_copper_chest", Map.of("into", "minecraft:weathered_copper_chest", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_copper_chest", Map.of("into", "minecraft:oxidized_copper_chest", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_copper_golem_statue", Map.of("into", "minecraft:copper_golem_statue", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_copper_golem_statue", Map.of("into", "minecraft:exposed_copper_golem_statue", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_copper_golem_statue", Map.of("into", "minecraft:weathered_copper_golem_statue", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_copper_golem_statue", Map.of("into", "minecraft:oxidized_copper_golem_statue", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_lightning_rod", Map.of("into", "minecraft:lightning_rod", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_lightning_rod", Map.of("into", "minecraft:exposed_lightning_rod", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_lightning_rod", Map.of("into", "minecraft:weathered_lightning_rod", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_lightning_rod", Map.of("into", "minecraft:oxidized_lightning_rod", "drops", new HashMap<String, Double>()));

            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_copper_chest", Map.of("into", "minecraft:copper_chest", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_copper_chest", Map.of("into", "minecraft:exposed_copper_chest", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_copper_chest", Map.of("into", "minecraft:weathered_copper_chest", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_copper_golem_statue", Map.of("into", "minecraft:copper_golem_statue", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_copper_golem_statue", Map.of("into", "minecraft:exposed_copper_golem_statue", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_copper_golem_statue", Map.of("into", "minecraft:weathered_copper_golem_statue", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_lightning_rod", Map.of("into", "minecraft:lightning_rod", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_lightning_rod", Map.of("into", "minecraft:exposed_lightning_rod", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_lightning_rod", Map.of("into", "minecraft:weathered_lightning_rod", "drops", new HashMap<String, Double>()));
        }
        if (PempekConfig.version < 45) {
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_copper_bars", Map.of("into", "minecraft:copper_bars", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_copper_bars", Map.of("into", "minecraft:exposed_copper_bars", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_copper_bars", Map.of("into", "minecraft:weathered_copper_bars", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_copper_bars", Map.of("into", "minecraft:oxidized_copper_bars", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_copper_chain", Map.of("into", "minecraft:copper_chain", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_copper_chain", Map.of("into", "minecraft:exposed_copper_chain", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_copper_chain", Map.of("into", "minecraft:weathered_copper_chain", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_copper_chain", Map.of("into", "minecraft:oxidized_copper_chain", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_copper_lantern", Map.of("into", "minecraft:copper_lantern", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_exposed_copper_lantern", Map.of("into", "minecraft:exposed_copper_lantern", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_weathered_copper_lantern", Map.of("into", "minecraft:weathered_copper_lantern", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.waxables.minecraft:waxed_oxidized_copper_lantern", Map.of("into", "minecraft:oxidized_copper_lantern", "drops", new HashMap<String, Double>()));

            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_copper_bars", Map.of("into", "minecraft:copper_bars", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_copper_bars", Map.of("into", "minecraft:exposed_copper_bars", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_copper_bars", Map.of("into", "minecraft:weathered_copper_bars", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_copper_chain", Map.of("into", "minecraft:copper_chain", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_copper_chain", Map.of("into", "minecraft:exposed_copper_chain", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_copper_chain", Map.of("into", "minecraft:weathered_copper_chain", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:exposed_copper_lantern", Map.of("into", "minecraft:copper_lantern", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:weathered_copper_lantern", Map.of("into", "minecraft:exposed_copper_lantern", "drops", new HashMap<String, Double>()));
            PempekConfig.config.set("world-settings.default.tools.axe.weatherables.minecraft:oxidized_copper_lantern", Map.of("into", "minecraft:weathered_copper_lantern", "drops", new HashMap<String, Double>()));
        }
        getMap("tools.axe.strippables", Map.ofEntries(
            ...
            if (block == Blocks.AIR) { PempekConfig.log(Level.SEVERE, "Invalid block for `tools.axe.strippables`: " + blockId); return; }
            if (!(obj instanceof Map<?, ?> map)) { PempekConfig.log(Level.SEVERE, "Invalid yaml for `tools.axe.strippables." + blockId + "`"); return; }
            String intoId = (String) map.get("into");
            Block into = BuiltInRegistries.BLOCK.getValue(Identifier.parse(intoId));
            if (into == Blocks.AIR) { PempekConfig.log(Level.SEVERE, "Invalid block for `tools.axe.strippables." + blockId + ".into`: " + intoId); return; }
            Object dropsObj = map.get("drops");
            if (!(dropsObj instanceof Map<?, ?> dropsMap)) { PempekConfig.log(Level.SEVERE, "Invalid yaml for `tools.axe.strippables." + blockId + ".drops`"); return; }
            ...
            dropsMap.forEach((itemId, chance) -> {
                ...
                if (item == Items.AIR) { PempekConfig.log(Level.SEVERE, "Invalid item for `tools.axe.strippables." + blockId + ".drops`: " + itemId); return; }
        ...
        getMap("tools.axe.waxables", Map.ofEntries(
            ...
            Block block = BuiltInRegistries.BLOCK.getValue(Identifier.parse(blockId));
            if (block == Blocks.AIR) { PempekConfig.log(Level.SEVERE, "Invalid block for `tools.axe.waxables`: " + blockId); return; }
            if (!(obj instanceof Map<?, ?> map)) { PempekConfig.log(Level.SEVERE, "Invalid yaml for `tools.axe.waxables." + blockId + "`"); return; }
            String intoId = (String) map.get("into");
            Block into = BuiltInRegistries.BLOCK.getValue(Identifier.parse(intoId));
            if (into == Blocks.AIR) { PempekConfig.log(Level.SEVERE, "Invalid block for `tools.axe.waxables." + blockId + ".into`: " + intoId); return; }
            Object dropsObj = map.get("drops");
            if (!(dropsObj instanceof Map<?, ?> dropsMap)) { PempekConfig.log(Level.SEVERE, "Invalid yaml for `tools.axe.waxables." + blockId + ".drops`"); return; }
            Map<Item, Double> drops = new HashMap<>();
            dropsMap.forEach((itemId, chance) -> {
                Item item = BuiltInRegistries.ITEM.getValue(Identifier.parse(itemId.toString()));
                if (item == Items.AIR) { PempekConfig.log(Level.SEVERE, "Invalid item for `tools.axe.waxables." + blockId + ".drops`: " + itemId); return; }
                drops.put(item, (double) chance);
            });
        ...
        getMap("tools.axe.weatherables", Map.ofEntries(
            ...
            Block block = BuiltInRegistries.BLOCK.getValue(Identifier.parse(blockId));
            if (block == Blocks.AIR) { PempekConfig.log(Level.SEVERE, "Invalid block for `tools.axe.weatherables`: " + blockId); return; }
            if (!(obj instanceof Map<?, ?> map)) { PempekConfig.log(Level.SEVERE, "Invalid yaml for `tools.axe.weatherables." + blockId + "`"); return; }
            String intoId = (String) map.get("into");
            Block into = BuiltInRegistries.BLOCK.getValue(Identifier.parse(intoId));
            if (into == Blocks.AIR) { PempekConfig.log(Level.SEVERE, "Invalid block for `tools.axe.weatherables." + blockId + ".into`: " + intoId); return; }
            Object dropsObj = map.get("drops");
            if (!(dropsObj instanceof Map<?, ?> dropsMap)) { PempekConfig.log(Level.SEVERE, "Invalid yaml for `tools.axe.weatherables." + blockId + ".drops`"); return; }
            Map<Item, Double> drops = new HashMap<>();
            dropsMap.forEach((itemId, chance) -> {
                Item item = BuiltInRegistries.ITEM.getValue(Identifier.parse(itemId.toString()));
                if (item == Items.AIR) { PempekConfig.log(Level.SEVERE, "Invalid item for `tools.axe.weatherables." + blockId + ".drops`: " + itemId); return; }
                drops.put(item, (double) chance);
            });
        ...
        getMap("tools.hoe.tillables", Map.ofEntries(
            ...
            Block block = BuiltInRegistries.BLOCK.getValue(Identifier.parse(blockId));
            if (block == Blocks.AIR) { PempekConfig.log(Level.SEVERE, "Invalid block for `tools.hoe.tillables`: " + blockId); return; }
            if (!(obj instanceof Map<?, ?> map)) { PempekConfig.log(Level.SEVERE, "Invalid yaml for `tools.hoe.tillables." + blockId + "`"); return; }
            String conditionId = (String) map.get("condition");
            Tillable.Condition condition = Tillable.Condition.get(conditionId);
            if (condition == null) { PempekConfig.log(Level.SEVERE, "Invalid condition for `tools.hoe.tillables." + blockId + ".condition`: " + conditionId); return; }
            String intoId = (String) map.get("into");
            Block into = BuiltInRegistries.BLOCK.getValue(Identifier.parse(intoId));
            if (into == Blocks.AIR) { PempekConfig.log(Level.SEVERE, "Invalid block for `tools.hoe.tillables." + blockId + ".into`: " + intoId); return; }
            Object dropsObj = map.get("drops");
            if (!(dropsObj instanceof Map<?, ?> dropsMap)) { PempekConfig.log(Level.SEVERE, "Invalid yaml for `tools.hoe.tillables." + blockId + ".drops`"); return; }
            Map<Item, Double> drops = new HashMap<>();
            dropsMap.forEach((itemId, chance) -> {
                Item item = BuiltInRegistries.ITEM.getValue(Identifier.parse(itemId.toString()));
                if (item == Items.AIR) { PempekConfig.log(Level.SEVERE, "Invalid item for `tools.hoe.tillables." + blockId + ".drops`: " + itemId); return; }
                drops.put(item, (double) chance);
            });
        ...
        getMap("tools.shovel.flattenables", Map.ofEntries(
            ...
            Block block = BuiltInRegistries.BLOCK.getValue(Identifier.parse(blockId));
            if (block == Blocks.AIR) { PempekConfig.log(Level.SEVERE, "Invalid block for `tools.shovel.flattenables`: " + blockId); return; }
            if (!(obj instanceof Map<?, ?> map)) { PempekConfig.log(Level.SEVERE, "Invalid yaml for `tools.shovel.flattenables." + blockId + "`"); return; }
            String intoId = (String) map.get("into");
            Block into = BuiltInRegistries.BLOCK.getValue(Identifier.parse(intoId));
            if (into == Blocks.AIR) { PempekConfig.log(Level.SEVERE, "Invalid block for `tools.shovel.flattenables." + blockId + ".into`: " + intoId); return; }
            Object dropsObj = map.get("drops");
            if (!(dropsObj instanceof Map<?, ?> dropsMap)) { PempekConfig.log(Level.SEVERE, "Invalid yaml for `tools.shovel.flattenables." + blockId + ".drops`"); return; }
            Map<Item, Double> drops = new HashMap<>();
            dropsMap.forEach((itemId, chance) -> {
                Item item = BuiltInRegistries.ITEM.getValue(Identifier.parse(itemId.toString()));
                if (item == Items.AIR) { PempekConfig.log(Level.SEVERE, "Invalid item for `tools.shovel.flattenables." + blockId + ".drops`: " + itemId); return; }
                drops.put(item, (double) chance);
            });
    ...
    private void bigDripleafSettings() {
        ...
        getMap("blocks.big_dripleaf.tilt-delay", Map.ofEntries(
            ...
            try {
                ...
                PempekConfig.log(Level.SEVERE, "Invalid big_dripleaf tilt key: " + tilt);
            }
        });
    }
    ...
    private void enderDragonSettings() {
        ...
        } else if (PempekConfig.version < 10) {
    ...
    private void villagerSettings() {
        ...
        if (PempekConfig.version < 9) {
            boolean oldValue = getBoolean("mobs.villager.lobotomize-1x1", villagerLobotomizeEnabled);
            set("mobs.villager.lobotomize.enabled", oldValue);
            set("mobs.villager.lobotomize-1x1", null);
        }
        if (PempekConfig.version < 27) {
