...
import org.stegripe.pempek.command.PempekCommand;
...
import org.stegripe.pempek.task.TPSBarTask;

@SuppressWarnings("unused")
public class PempekConfig {
    private static final String HEADER = "This is the main configuration file for Pempek.\n"
            + "As you can see, there's tons to configure. Some options may impact gameplay, so use\n"
            + "with caution, and make sure you know what each option does before configuring.\n"
            + "\n"
            + "If you need help with the configuration or have any questions related to Pempek,\n"
            + "join us in our Discord guild.\n"
            + "\n"
            + "Website: https://pempek.stegripe.org \n"
            + "Docs: https://pempek.stegripe.org/docs \n";
    ...
    public static void init(File configFile) {
        ...
        try {
            ...
            Bukkit.getLogger().log(Level.SEVERE, "Could not load pempek.yml, please correct your syntax errors", ex);
        ...
        commands.put("pempek", new PempekCommand("pempek"));

        version = getInt("config-version", 48);
        set("config-version", 48);

        readConfig(PempekConfig.class, null);
    ...
    public static void registerCommands() {
        for (Map.Entry<String, Command> entry : commands.entrySet()) {
            MinecraftServer.getServer().server.getCommandMap().register(entry.getKey(), "Pempek", entry.getValue());
        }
    }
