...
public class UptimeCommand {
    ...
    private static int execute(CommandSourceStack sender) {
        ...
        data.format = PempekConfig.uptimeFormat;
        data.hide = true;
        data.millis = System.currentTimeMillis() - MinecraftServer.startTimeMillis;

        process(data, "<days>", PempekConfig.uptimeDay, PempekConfig.uptimeDays, TimeUnit.DAYS, TimeUnit.MILLISECONDS::toDays);
        process(data, "<hours>", PempekConfig.uptimeHour, PempekConfig.uptimeHours, TimeUnit.HOURS, TimeUnit.MILLISECONDS::toHours);
        process(data, "<minutes>", PempekConfig.uptimeMinute, PempekConfig.uptimeMinutes, TimeUnit.MINUTES, TimeUnit.MILLISECONDS::toMinutes);
        data.hide = false; // never hide seconds
        process(data, "<seconds>", PempekConfig.uptimeSecond, PempekConfig.uptimeSeconds, TimeUnit.SECONDS, TimeUnit.MILLISECONDS::toSeconds);

        Component output = MiniMessage.miniMessage().deserialize(PempekConfig.uptimeCommandOutput, Placeholder.unparsed("uptime", data.format));
