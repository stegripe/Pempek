Contributing to Pempek
==========================
Pempek is happy you're willing to contribute to our projects. We are usually
very lenient with all submitted PRs, but there are still some guidelines you
can follow to make the approval process go more smoothly.

## Use a Personal Fork and not an Organization

Pempek will routinely modify your PR, whether it's a quick rebase or to take care
...
Unfortunately, if you use an organization for your PR, it prevents Pempek from
...
- A Java 21 or later JDK (packages vary, use Google/DuckDuckGo/etc.).
    ...
    - Pempek requires JDK 21 to build, however, makes use of Gradle's
...
Unlike the Pempek API and its implementation, modifications to Paper and Minecraft source files
...
- `minecraft-patches` (applies to the `pempek-server//` git repo)
...
3. cd into `pempek-server` for server changes, `pempek-api` for API changes,
   `paper-api` for Paper API changes, and `paper-server` for Paper Server changes.

`pempek-server/src/minecraft/java` and `pempek-server/src/minecraft/java/resources` are not git repositories in the traditional sense.
...
- `./gradlew rebuildPaperSingleFilePatches` - Fixups and rebuilds all paper single-file patches. This is how you'd make changes to the `build.gradle.kts` files located under `pempek-api` and `pempek-server`
...
1. Replace `pick` with `edit` for the commit/patch you want to modify (in this
   case the very first commit, `pempek File Patches`), and
   "save" the changes;
...
optimizations we have. This makes it easier to update Pempek during Minecraft updates,
...
Your commit will be converted into a patch that you can then PR into Pempek.
...
These steps assume the `origin` remote is your fork of this repository and `upstream` is the official Stegripe repository.
...
Remember that these changes will affect everyone who runs Pempek, not just you
and your server.
...
- You need to add a comment with a short and identifiable description of the patch:
  `// Pempek start - <COMMIT DESCRIPTION>`
...
- Multi-line changes start with `// Pempek start - <COMMIT DESCRIPTION>` and end
  with `// Pempek end - <COMMIT DESCRIPTION>`.
- One-line changes should have `// Pempek - <COMMIT DESCRIPTION>` at the end of the line.
...
Here's an example of how to mark changes by Pempek:

```java
entity.getWorld().dontBeStupid(); // Pempek - Move away from beStupid()
entity.getFriends().forEach(Entity::explode);
entity.updateFriends();

// Pempek start - Use plugin-set spawn
// entity.getWorld().explode(entity.getWorld().getSpawn());
Location spawnLocation = ((CraftWorld) entity.getWorld()).getSpawnLocation();
entity.getWorld().explode(new BlockPosition(spawnLocation.getX(), spawnLocation.getY(), spawnLocation.getZ()));
// Pempek end - Use plugin-set spawn
```
...
public class SomeVanillaClass {
    public final org.bukkit.Location newLocation; // Pempek - add location
}
```
...
but the visibility is too low (e.g. a private field in an entity class). Pempek can use access transformers
to change the visibility or remove the final modifier from fields, methods, and classes. Inside the `build-data/pempek.at`
...
When submitting feature patches to Pempek, we may ask you to add notes to the patch
...
to make future updates easier on ourselves, Pempek tries to use obfuscation
...
```java
double d0 = entity.getX(); final double fromX = d0; // Pempek - OBFHELPER
// ...   
this.someMethod(fromX); // Pempek
```
...
`PempekConfig` or `PempekWorldConfig` classes. Use `PempekConfig` if a value
...
```java
return this.level.pempekConfig.useInhabitedTime ? this.inhabitedTime : 0;
```

## Testing API changes

### Using the Pempek Test Plugin

The Pempek project has a `test-plugin` module for easily testing out API changes
...
of the other Pempek run tasks).

### Publishing to Maven local (use in external plugins)

To build and install the Pempek APIs and Server to your local Maven repository, do the following:
...
- Add `mavenLocal()` as a repository. Gradle checks repositories in the order they are declared,
  so if you also have the Pempek repository added, put the local repository above Pempek's.
...
- If you later need to use the Pempek-API, you might want to remove the jar
